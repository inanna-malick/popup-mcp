...
use crate::models::{PopupState, PopupResult};
use std::fs;
...
fn test_parse_example_files() {
    ...
    for entry in fs::read_dir(examples_dir).unwrap() {
        ...
        if path.extension().and_then(|s| s.to_str()) == Some("json") {
            ...
            assert!(!popup.title.is_empty(), "Title should not be empty in {:?}", path);
...
fn test_popup_state_initialization() {
    ...
    assert!(state.get_multichoice_mut("Features").unwrap().iter().all(|&x| !x));
}
