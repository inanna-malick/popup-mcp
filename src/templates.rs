...
pub fn load_templates() -> Result<Vec<LoadedTemplate>> {
    ...
    let config: TemplateConfig = toml::from_str(&config_str)
        .map_err(|e| anyhow!("Failed to parse popup.toml: {}", e))?;
...
pub fn instantiate_template(
    ...
    parse_popup_json(&json_str)
        .map_err(|e| anyhow!("Generated invalid JSON from template: {}", e))
}
...
pub fn generate_tool_schema(template: &Template) -> Value {
    ...
    for (name, param) in &template.params {
        ...
        prop.insert(
            "type".to_string(),
            Value::String(match param.param_type {
